...
#define neurolib_logistic_der(x) (intervalend-intervalstart)*((1.0f - neurolib_logistic(x))* neurolib_logistic(x))
#define neurolib_soft_plus_der(x) (1.0f / (1.0f + std::exp(-x)))
#define neurolib_tanh_der(x) (1.0f - std::pow(neurolib_tanh(x), 2))

//Useful constructions
void calc_activation_function(const float* src, size_t size, ActivationFunctionType af, float* dest, int intervalstart, int intervalend);
void calc_activation_derivatives(const float* src, size_t size, ActivationFunctionType af, float* dest, int intervalstart, int intervalend);
